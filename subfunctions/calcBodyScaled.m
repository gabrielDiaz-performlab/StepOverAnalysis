function [ sessionData ] = calcBodyScaled(sessionData, trIdx)
...
rFootObsCentered_fr_XYZ(:,2) =  rawData.obstacle_XposYposHeight(2) - rFootObsCentered_fr_XYZ(:,2);
...
lFootObsCentered_fr_XYZ(:,2) =  rawData.obstacle_XposYposHeight(2) - lFootObsCentered_fr_XYZ(:,2);
lFootObsCentered_fr_XYZ = lFootObsCentered_fr_XYZ ./ repmat(legLength,size(lFootObsCentered_fr_XYZ));

%%

sessionData.processedData_tr(trIdx).rFootBS_fr_XYZ = rFootObsCentered_fr_XYZ;
sessionData.processedData_tr(trIdx).lFootBS_fr_XYZ = lFootObsCentered_fr_XYZ;
